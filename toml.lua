...
--new read method
--reads from dedicated toml/data file directory
--and returns the parsed result
TOML.read = function(file, options)
	
	local f = io.open(get_data_file_path(file .. '.toml'), "rb")
	local toml = f:read("*a")
	f:close()
	
	return TOML.parse(toml, options)
end
...
TOML.parse = function(toml, options)
	...
	local function err(message, strictOnly)
		if not strictOnly or (strictOnly and strict) then
			...
			error("TOML_ERROR: " .. message .. " on line " .. line .. ".", 4)
		end
	end
